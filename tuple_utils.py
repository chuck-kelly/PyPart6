...
def print_indexes_and_entries(indexes: Iterable, entries: Iterable) -> None:
    ...
    :return: None
    """
    for matching_index_places in zip(indexes,entries):
        ten_spaces=10
        indexes_lenght= len(str(matching_index_places[0]))
        spaces_needed =ten_spaces-indexes_lenght
        space_str = ''
        for x in range(spaces_needed-1):
            space_str = space_str + ' '
        print('Index:',matching_index_places[0],space_str,'Entry:' ,matching_index_places[1])
...
def print_items_with_index(items: Iterable):
    ...
    :return: None
    """
    for index_of_items, element_in_items in enumerate(items, start=1):
        print(str(index_of_items)+':',element_in_items)
